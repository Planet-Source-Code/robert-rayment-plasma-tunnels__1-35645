...
;Public Type PStruc
;   PtrIndexArray As Long      ' byte array (1-SqSize,1-SqSize) values 0-255
;   PtrColArray As Long        ' byte array (1-SqSize,1-SqSize) values 0-255
;   PtrLineStore As Long       ' byte array (1-SqSize) values 0-255
;   PtrCoordsX As Long         ' integer array (1-SqSize) values 1-SqSize
;   PtrCoordsY As Long         ' integer array (1-SqSize) values 1-SqSize
...
;' Fixed values
;' sizex As Long           ' SqSize
;' sizey As Long           ' SqSize
...
NoiseDone:
    mov eax,iy
    add eax,Stepsize
    cmp eax,SqSize          ; iy+Stepsize <= sizey
    jb Tix
...
fillCol:    
    ...
    Fy:
        ...
        cmp ecx,SqSize         ; If ky <= SqSize Then
        ja Ny
            mov ecx,ix
        Fx:
            cmp ecx,SqSize     ; If kx <= SqSize then
            ja Ny
...
NIX:
    pop ecx
    add ecx,Stepsize
    cmp ecx,SqSize             ; ix <= sizex
    jbe near FIX
NIY:
    pop ecx
    add ecx,Stepsize
    cmp ecx,SqSize             ; iy <= sizey
    jbe near FIY
...
;-----------------
    ...
    mov ecx,SqSize             ; iy=sizey
SAy:
    push ecx
    mov ky,ecx
    
    mov ecx,SqSize             ; ix=sizex
SAx:
    mov kx,ecx
    Call near IndexArrayAddr    ; edi->addr
    mov BL,[edi]
...
Getzmul:
    ...
    mov ecx,SqSize             ; sizey=SqSize
CAy:
    push ecx
    mov ky,ecx
    
    mov ecx,SqSize             ; sizex=SqSize
CAx:
    mov kx,ecx
    Call near IndexArrayAddr    ; edi->addr
    mov BL,[edi]
...
noadd2pi:
    ...
    faddp st1           ; SqSize
    fld dword zAngle
    fmulp st1           ; SqSize*zAngle
    fldpi
    fldpi
    faddp st1           ; 2*pi,SqSize*zAngle
    fdivp st1           ; st1/st0 = SqSize*zAngle/2*pi
    ...
    faddp st1           ; (SqSize*zAngle/2*pi)+0.2
    fistp dword ixs     ; ixs = (SqSize*zAngle/2*pi)+0.2
...
ScrD:
    ...
    dec ebx
    add eax,ebx
    add edi,eax     ; edi -> IndexArray(1,SqSize)
...
RET
;===========================
CoordsXAddr:  ; Addr = edi + 2*[(ky-1) * SqSize + (kx-1)]
...
RET
;===========================
CoordsYAddr:  ; Addr = edi + 2*[(ky-1) * SqSize + (kx-1)]
